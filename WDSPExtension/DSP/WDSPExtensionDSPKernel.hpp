...
#import "WDSPExtensionParameterAddresses.h"

/*
 WDSPExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class WDSPExtensionDSPKernel {
public:
    ...
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case WDSPExtensionParameterAddress::gain:
                mGain = value;
                break;
                // Add a case for each parameter in WDSPExtensionParameterAddresses.h
        }
    }
    ...
    void process(std::span<float const*> inputBuffers, std::span<float *> outputBuffers, AUEventSampleTime bufferStartTime, AUAudioFrameCount frameCount) {
        /*
         ...
         modify the check in [WDSPExtensionAudioUnit allocateRenderResourcesAndReturnError]
         */
